...
export const authApi = {
  ...
  async confirmTwoFactor(code: string): Promise<{ backupCodes: string[] }> {
    const response = await apiClient.post<ApiResponse<{ backupCodes: string[] }>>('/auth/2fa/confirm', {
      code,
    })
    return handleApiResponse(response)
  },
  ...
  async disableTwoFactor(password: string): Promise<void> {
    const response = await apiClient.post<ApiResponse<void>>('/auth/2fa/disable', { password })
    return handleApiResponse(response)
  },
